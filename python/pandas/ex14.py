...
fig.add_trace(
    go.Pie(labels=countries, values=ghg, name="온실가스 Emissions"),
    # 1행 1열에 위치
    row=1,
    col=1,
)
...
fig.add_trace(
    go.Pie(labels=countries, values=co2, name="CO2 Emissions"),
    # 1행 2열에 위치
    row=1,
    col=2,
)
...
fig.update_layout(
    ...
    annotations=[
        dict(text="온실가스", x=0.19, y=0.5, font_size=20, showarrow=False),
