...
import com.study.LibraryStie.entity.snsUser.SnsUserRepository;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    private final HttpSession httpSession;          // 세션 저장용
    private final SnsUserRepository snsUserRepository; // DB 저장용

    @Override
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {

        // 1. 기본 OAuth2 서비스로 플랫폼에서 사용자 정보 가져오기
        OAuth2UserService<OAuth2UserRequest, OAuth2User> delegate = new DefaultOAuth2UserService();
        OAuth2User oAuth2User = delegate.loadUser(userRequest);

        // 2. 어떤 플랫폼인지 확인 (kakao, naver)
        String registrationId = userRequest.getClientRegistration().getRegistrationId();

        // 3. 각 플랫폼의 사용자 식별 키 이름 가져오기
        String userNameAttributeName = userRequest.getClientRegistration()
                .getProviderDetails().getUserInfoEndpoint().getUserNameAttributeName();

        // 4. 플랫폼별 응답 구조를 통일된 형태로 변환
        OAuthAttributes attribute = OAuthAttributes.of(
                registrationId, userNameAttributeName, oAuth2User.getAttributes());

        // 5. DB에 저장 또는 업데이트 (신규 회원 가입 / 기존 회원 정보 갱신)
        SnsUser user = saveOrUpdate(attribute);

        // 6. 세션에 사용자 정보 저장 (엔티티 직접 저장 금지 -> SessionUser DTO 사용)
        httpSession.setAttribute("user", new SessionUser(user));

        // 7. Spring Security 인증 객체 반환
    ...
    private SnsUser saveOrUpdate(OAuthAttributes attr) {
        ...
        SnsUser snsUser = snsUserRepository.findByProviderAndProviderId(provider, providerId)
                .map(entity -> entity.update(name, picture, email))  // 기존 회원 -> 정보 업데이트
                .orElse(SnsUser.builder()                            // 신규 회원 -> 새로 생성
                        .name(name)
                        .email(email)
                        .picture(picture)
                        .provider(provider)
